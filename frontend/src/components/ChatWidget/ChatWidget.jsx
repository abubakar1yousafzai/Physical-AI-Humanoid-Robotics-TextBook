...
const ChatWidget = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      if (isAuthenticated) {
        try {
          const response = await fetch('https://abu-bakar1yousafzai-deploy-backend.hf.space/api/chat/history', {
            headers: {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const response = await fetch('https://abu-bakar1yousafzai-deploy-backend.hf.space/api/chat', {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
          message: text,
